...
class PagamentoViewSimplificada:
    def __init__(self):
    ...
    def menu_formas_de_pagamento_interno(self):
        while True:
            ...
            if opcao == "1":
                ...
                try:
                    id_fp = int(input("ID da Forma de Pagamento para buscar: "))
    ...
    def menu_pagamentos_interno(self):
        while True:
            ...
            if opcao == "1":
                ...
                try:
                    val = float(input("Valor (ex: 50.99): R$"))
                    enc_id = int(input("ID da Encomenda: "))
                    ...
                    fp_id = int(input("ID da Forma de Pagamento: "))
                    self.p_controller.criar_pagamento(val, enc_id, fp_id)
                except ValueError:
                    print("Erro: Entrada inválida para valor ou IDs.")
            elif opcao == "2":
                ...
                try:
                    id_p = int(input("ID do Pagamento para consultar: "))
                ...
                try:
                    id_conf = int(input("ID do Pagamento para CONFIRMAR: "))
                    self.p_controller.confirmar_pagamento(id_conf)
                except ValueError:
                    print("ID inválido.")
            elif opcao == "5": 
                try:
                    id_canc = int(input("ID do Pagamento para CANCELAR: "))
                    self.p_controller.cancelar_pagamento(id_canc)
                except ValueError:
                    print("ID inválido.")
            elif opcao == "6": 
                try:
                    id_fail = int(input("ID do Pagamento para marcar como FALHO: "))
...
if __name__ == "__main__":
    ...
    print("\n--- Teste do Menu de Pagamentos Simplificado ---")
    view_teste = PagamentoViewSimplificada()
