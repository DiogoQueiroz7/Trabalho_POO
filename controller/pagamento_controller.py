...
class PagamentoController:
    ...
    def criar_pagamento(self, valor: float, encomenda_id: int, forma_pagamento_id: int) -> Pagamento | None: #
        """
        Cria um novo pagamento, validando os dados e salvando através do repositorio
        """

        if not isinstance(valor, (float, int)) or valor <= 0: 
            print("Erro: O valor do pagamento deve ser um número positivo.")
            return None
    ...
    def listar_todos_os_pagamentos(self) -> list[Pagamento]: 
    ...
    def consultar_pagamento_por_id(self, id_pag: int) -> Pagamento | None: 
    ...
    def _processar_mudanca_status(self, pagamento_id: int, acao: str) -> bool:
        """
        Lógica centralizada para mudar o status de um pagamento.
        'acao' pode ser 'confirmar', 'cancelar' ou 'falhar'.
        """
        ...
        if sucesso_na_acao_modelo:
            ...
            if sucesso_no_banco:
                ...
                print(f"Erro CRÍTICO: Status do Pagamento ID {pagamento_id} foi alterado no objeto, mas falhou ao salvar no banco de dados!")
                return False
        else:
            return False
    ...
    def marcar_pagamento_como_falho(self, pagamento_id: int) -> bool:
        """
        Tenta marcar um pagamento como falho.
        """
        if not isinstance(pagamento_id, int) or pagamento_id <= 0:
